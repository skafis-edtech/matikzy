function parseTicks(tickStr, min, max) {
  ...
  return tickStr
    ...
    .map((s) => {
      const eq = s.indexOf("=");
      if (eq !== -1)
        return { value: s.slice(0, eq).trim(), label: s.slice(eq + 1).trim() };
...
function evalNum(s) {
  ...
  return slash !== -1
    ? (sign * parseFloat(abs.slice(0, slash))) /
        parseFloat(abs.slice(slash + 1))
    : parseFloat(s);
}
...
function parseLinearExpr(expr) {
  ...
  if (mxMatch) {
    return {
      m: evalNum(mxMatch[1] || "1"),
      b: mxMatch[2] ? evalNum(mxMatch[2]) : 0,
    };
  }
...
function extractTransforms(line) {
  ...
  const cleanLast = last.slice(
    0,
    last.length - (rangesM ? rangesM[0].length : 0),
  );
  const trParts = [...parts.slice(1, -1), cleanLast];
  return {
    clean,
    transforms: trParts.map(parseOneTransform).filter(Boolean),
    postRanges,
  };
}
...
function parseGraphLine(line) {
  ...
  const eqMatch = line.match(
    new RegExp(`^graph\\s+line\\s+y=([^\\s]+)${RANGES_RE.source}`),
  );
  if (eqMatch) {
  ...
  const ptMatch = line.match(
    new RegExp(
      `^graph\\s+line\\s+\\(([^;]+);([^)]+)\\)\\s+\\(([^;]+);([^)]+)\\)${RANGES_RE.source}`,
    ),
  );
  if (ptMatch) {
    const x1 = parseFloat(ptMatch[1]),
      y1 = parseFloat(ptMatch[2]);
    const x2 = parseFloat(ptMatch[3]),
      y2 = parseFloat(ptMatch[4]);
  ...
  const vMatch = line.match(
    new RegExp(`^graph\\s+line\\s+x=([^\\s]+)${RANGES_RE.source}`),
  );
  if (vMatch) {
...
function parseParabolaLine(line) {
  const baseMatch = line.match(
    new RegExp(`^graph\\s+parabola(?:\\s+(.*?))?${RANGES_RE.source}$`),
  );
...
function parseCubicLine(line) {
  const baseMatch = line.match(
    new RegExp(`^graph\\s+cubic(?:\\s+(.*?))?${RANGES_RE.source}$`),
  );
  if (!baseMatch) return null;
...
function parseLogLine(line) {
  const m = line.match(
    new RegExp(`^graph\\s+log(?:\\s+a=([^\\s]+))?${RANGES_RE.source}$`),
  );
  if (!m) return null;
...
function parseExpLine(line) {
  const m = line.match(
    new RegExp(`^graph\\s+exp(?:\\s+a=([^\\s]+))?${RANGES_RE.source}$`),
  );
  if (!m) return null;
  const a = m[1] ? evalNum(m[1]) : 2;
  if (isNaN(a) || a <= 0 || a === 1) return null;
...
function parseTrigLine(line) {
  const m = line.match(
    new RegExp(`^graph\\s+(sin|cos|tan|tg|cot|ctg)${RANGES_RE.source}$`),
  );
  if (!m) return null;
  const raw = m[1];
...
function parseGenericLine(line) {
  ...
  // --- NEW: function mode ---
  if (body.startsWith("y=")) {
    const expr = body.slice(2).trim();
    if (!expr) return null;
    return { generic: true, smooth, expr, ...ranges };
  }
...
function parseHyperbolaLine(line) {
  const baseMatch = line.match(
    new RegExp(`^graph\\s+hyperbola\\s+(.*?)${RANGES_RE.source}$`),
  );
  if (!baseMatch) return null;
...
function syntaxCheck(content, defaultExtent = 3) {
  ...
  if (/\bgraph\s+circle\b.*[a-zA-Z]\(/.test(content))
    return {
      valid: false,
      errors: [
        'Circle center must be "(x;y)" with no letter before the parenthesis',
      ],
    };
...
function compile(content, grid = false, defaultExtent = 3, tikzScale = 1) {
  ...
  const applyXTr = (x, tr) => {
    for (const t of tr) {
      if (t.type === "hshift") x += t.value;
      else if (t.type === "hscale") x *= t.value;
      else if (t.type === "hflip") x = -x;
    }
    return x;
  };
  const invXTr = (x, tr) => {
    for (let i = tr.length - 1; i >= 0; i--) {
      const t = tr[i];
      if (t.type === "hshift") x -= t.value;
      else if (t.type === "hscale" && t.value !== 0) x /= t.value;
      else if (t.type === "hflip") x = -x;
    }
    return x;
  };
  const applyYTr = (y, tr) => {
    for (const t of tr) {
      if (t.type === "vshift") y += t.value;
      else if (t.type === "vscale") y *= t.value;
      else if (t.type === "neg") y = -y;
      else if (t.type === "abs") y = Math.abs(y);
    }
    return y;
  };
  ...
  if (graphs.length > 0 || areas.length > 0) {
    ...
    if (areas.length > 0) {
      ...
      const mark = (c, r) => {
        if (c >= 0 && c < GW && r >= 0 && r < GH) wallGrid[r * GW + c] = 1;
      };
      ...
      for (const g of graphs) {
        ...
        if (g.vertical) {
          const sc = xToC(apX(g.x));
          for (let r = 0; r < GH; r++) mark(sc, r);
          continue;
        }
        if (g.circle) {
          let pc = null,
          ...
          for (let i = 0; i <= SAMP; i++) {
            const th = (2 * Math.PI * i) / SAMP,
              c = xToC(g.cx + g.r * Math.cos(th)),
              r = yToR(g.cy + g.r * Math.sin(th));
            if (pc !== null) bres(pc, pr, c, r);
            else mark(c, r);
            pc = c;
            pr = r;
          }
        ...
        let pc = null,
          pr = null;
        for (let i = 0; i <= SAMP; i++) {
          const sx = xStart + ((xEnd - xStart) * i) / SAMP,
            sy = evalY(sx);
      ...
      for (const ag of areas) {
        ...
        while (qH < qT) {
          ...
          if (r < GH - 1 && grid[(r + 1) * GW + c] === 0) {
            grid[(r + 1) * GW + c] = 2;
            q[qT++] = (r + 1) * GW + c;
          }
        }
        // Collect per-row left/right boundaries and centroid
        const lB = new Int32Array(GH).fill(-1),
          rB = new Int32Array(GH).fill(-1);
        let sumC = 0,
          sumR = 0,
          cnt = 0,
          mnR = GH,
    ...
    for (const g of graphs) {
      ...
      const domX = () => {
        let lo = xLoEff ?? xStart;
        let hi = xHiEff ?? xEnd;

        if (!isFinite(lo)) lo = xStart;
        if (!isFinite(hi)) hi = xEnd;
      ...
      if (g.parabola) {
        ...
        if (!tr.length) {
          ...
          const solveY = (yB) => {
            ...
            return [(-g.b - sq) / (2 * g.a), (-g.b + sq) / (2 * g.a)].sort(
              (u, v) => u - v,
            );
          };
          const cr = g.a > 0 ? solveY(yHi) : solveY(yLo);
          if (cr) {
            xLo = Math.max(xLo, cr[0]);
            xHi = Math.min(xHi, cr[1]);
          }
        }
        if (xLo >= xHi) continue;
      ...
      if (g.trig) {
        ...
        const drawBranches = (baseExpr, isAsymptote) => {
          ...
          const eps = 0.04;
          const fLo = applyXTr(xLo, tr),
            fHi = applyXTr(xHi, tr);
          const fMin = Math.min(fLo, fHi),
            fMax = Math.max(fLo, fHi);
          const asyms = [];
          for (let n = Math.floor(fMin) - 2; n <= Math.ceil(fMax) + 2; n++)
            if (isAsymptote(n)) {
              const sx = invXTr(n, tr);
      ...
      if (g.sqrt) {
        ...
        for (const t of tr) {
          if (t.type === "hshift") {
            b += t.value;
          } else if (t.type === "hscale") {
            a *= t.value;
            b *= t.value;
          } else if (t.type === "hflip") {
            a = -a;
            b = -b;
          }
        }
