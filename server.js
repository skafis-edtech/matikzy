...
async function inc(key) {
    if (redis) {
        try { await redis.incr("stats:" + key); } catch {}
    }
}
...
app.get("/stats", async (_, res) => {
    ...
    try {
        ...
        const stats = Object.fromEntries(keys.map((k, i) => [k, Number(values[i] ?? 0)]));
        res.json(stats);
    } catch {
...
function checkTikzSyntax(code) {
    ...
    if (!code.includes("\\begin{tikzpicture}")) return "Missing \\begin{tikzpicture}";
...
function withRenderLock(fn) {
    ...
    const timedFn = () => new Promise((resolve, reject) => {
        ...
        fn().then(
            (v) => { queueDepth--; clearTimeout(deadline); resolve(v); },
            (e) => { queueDepth--; clearTimeout(deadline); reject(e); }
        );
    });
...
app.post("/render", limiter, async (req, res) => {
    ...
    if (!lang || !content) {
        res.status(400).send('Body must be {"lang": "tikz"|"matikzy", "content": "..."}');
        return;
    }
    ...
    const ip = req.ip;
    console.info("[render] received", { ip, chars: tikz.length, key: key.slice(0, 16) });
    inc("requests");
    ...
    try {
        ...
        console.error("[render] failed", { ip, ms: Date.now() - start, error: err.message });
